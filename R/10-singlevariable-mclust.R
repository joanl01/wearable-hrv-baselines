...
hrv_clustered <- run_mclust_hrv(
  hrv_labeled,
  metrics = hrv_metrics
)

# Force exactly 2 clusters
hrv_clustered_2 <- run_mclust_hrv(
  hrv_labeled,
  metrics = hrv_metrics,
  G = 2,
  suffix = "_2"
...
# plot and save for all participants for automatic clusters
for (person in participants) {
  for (hrvar in hrv_metrics){
    plot_mclust_metric(hrv_clustered, person, hrvar, cluster_suffix = "") 
    plot_mclust_metric_histogram(hrv__clustered, person, hrvar, cluster_suffix = "") 
  }}


# plot and save for all participants for 2 forced clusters
for (person in participants) {
  for (hrvar in hrv_metrics){
    plot_mclust_metric(hrv_clustered_2, person, hrvar, cluster_suffix = "_2") 
    plot_mclust_metric_histogram(hrv_clustered_2, person, hrvar, cluster_suffix = "_2") 
  }}
