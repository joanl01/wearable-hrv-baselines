...
sleep_transition_data_all <-  hrv_with_sleep_all |> 
...
train <- subset(sleep_transition_data_all, sample == TRUE)
test  <- subset(sleep_transition_data_all, sample == FALSE)
...
df_all_test |> ggplot( aes(x = FPR, y = TPR, color = Model)) +
  ...
  labs(
    title = "ROC curve for Backward, Bidirectional, and Forward selection models",
    x = "False Positive Rate",
    y = "True Positive Rate"
  )  + theme_minimal() + facet_wrap(~Model) + theme(legend.position = "none", text = element_text(size = 15)) 


ggsave(here::here("figures", "Chapter 5", "p7-logistic-regression-roc-plot.pdf"),
       plot = last_plot(),
