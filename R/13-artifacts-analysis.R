...
plot_art <- artifacts_combined_results %>% na.omit() %>% filter(DIV< Inf) %>% select(where(is.numeric)) %>% 
  select(-"Time", -"shift", -"sizesp", -"ULFmin", -"ULFmax",
         -"VLFmin", -"VLFmax", -"LFmin", -"LFmax", -"HFmin",
         -"HFmax") 

pairs_art <- plot_art |> select(SDANN, rMSSD, TINN, HRVi, DET, REC)|> ggpairs() + theme_classic() + 
  theme(text= element_text(size = 15)) +
  labs(title = "Pairwise plot for five lowest artifact days")

ggsave(here::here("figures", "Chapter 6", "Correlation","artifacts","artifacts-pairs-plot.pdf"),
       width = 16, 
       height = 9, 
       units = "in",
       dpi = 500,
...
hclust_art<- hclust(dist(t(as.matrix(plot_art)))) %>%
  ggdendrogram(size = 5, rotate = FALSE) + labs(
    title = "Cluster Dendrogram of all baseline measures using lowest artifact days"
  ) + theme(text = element_text(size = 20))

ggsave(here::here("figures", "Chapter 6", "Correlation" ,"artifacts", "artifacts-hclust.pdf"),
       plot = hclust_art, 
       width = 16, 
       height = 9,
       dpi = 500,
...
artifacts_corrs <- artifacts_combined_results %>%
...
artifacts_corrs <- artifacts_corrs |>
...
artifact_unique <- artifacts_corrs |>
  mutate(
...
unique_measures <- artifacts_corrs$var1 |> unique()


artifact_corr_all <- artifacts_combined_results |> get_corr()

artifact_corr_all <- artifact_corr_all |>
  mutate(comp = str_glue("{var1}={var2}")) |> 
  filter(!var1 %in% c("Time", "shift", "sizesp", "ULFmin", 
                      "ULFmax", "VLFmin", "VLFmax", "LFmin","LFmax", "HFmin","HFmax", "DIV")) |> 
  filter(!var2 %in% c("Time", "shift", "sizesp", "ULFmin", 
...
artifact_corr_all_unique <- artifact_corr_all |>
  mutate(
    var_min = pmin(var1, var2),
    var_max = pmax(var1, var2)
  ) |>
...
heatmap_art <- heat_df|> 
  ...
  labs(title = "Correlation heatmap of lowest artifact days",
       x = "Variable 1",
       y = "Variable 2",
       fill = "Correlation"
  )
...
ggsave(here::here("figures", "Chapter 6", "Correlation", "artifacts", "artifacts-heatmap-all.pdf"),
       height = 10, 
       width = 10,
       units = "in",
       plot = heatmap_art,
...
combinations %>%
  pwalk(function(participant, timeline, days) {
    ...
    filename <- here::here(
      ...
      "artifacts",
      paste0("artifacts-", participant, "-", gsub(" ", "-", timeline), ".pdf")
    )
...
clean_df <- artifacts_combined_results |> select(-DIV) |> na.omit()|> 
  select(-"Time", -"shift", -"sizesp", -"ULFmin", 
         -"ULFmax", -"VLFmin", -"VLFmax", -"LFmin",
         -"LFmax", -"HFmin",-"HFmax")


for (participant in unique(artifacts_combined_results$participant)) {
  ...
  if (nrow(deep_sleep) > 1) {
    ...
    ggsave(here::here("figures", "Chapter 6", "Correlation", "artifacts", paste0("artifacts-deep-sleep-hclust-",participant,".pdf")),
           plot = deep_sleep,
           height = 10, 
           width = 10, 
           units = "in",
  ...
  if (nrow(fmb4w) > 1) {
    ...
    ggsave(here::here("figures", "Chapter 6", "Correlation", "artifacts",paste0("artifacts-first-before-hclust-",participant,".pdf")),
           plot = first_before,
           height = 10, 
           width = 10, 
           units = "in",
  ...
  if (nrow(fmafw) > 1) {
    ...
    ggsave(here::here("figures", "Chapter 6", "Correlation", "artifacts", paste0("artifacts-first-after-hclust-",participant,".pdf")),
           plot = first_after,
           height = 10, 
           width = 10, 
           units = "in",
  ...
  if (nrow(awake) > 1) {
    ...
    ggsave(here::here("figures", "Chapter 6", "Correlation", "artifacts", paste0("artifacts-rand-awake-hclust-",participant,".pdf")),
           plot = rand_awake,
           height = 10, 
           width = 10, 
           units = "in",
